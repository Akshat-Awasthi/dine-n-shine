...
const Dashboard = () => {
    const [headerfocus,setHeaderfocus] = useState(null);
    const dashboardHeader = [{id : 1, type : 'All'},{id : 2, type : 'New Orders'},{id : 3, type : 'Dine in'},{id : 4, type : 'Take Away'},];
  return (
        ...
        <div>
            {
                dashboardHeader.map((item)=> (
                    <span key={item.id} onClick={()=>setHeaderfocus(item.id)} className={`h-5 w-10 m-4 p-3 hover:cursor-pointer transform  focus:text-black rounded-md ${headerfocus===item.id ? 'text-black bg-yellow-300': 'text-gray-500 bg-white'}`}>{item.type}</span>
                ))
            }
        </div>
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 h-full w-[1200px] mt-5 ml-2'>
            {/* ALL ORDERS */}
                {headerfocus === 1 && orderData.orders.map((item) => (
                    <div key={item.id} className="w-11/12 h-96 p-4 mt-2 bg-white rounded-md shadow">
                        ...
                        <div className=" mt-4 flex justify-between">
                        <button className="bg-blue-500 text-sm text-white p-2 rounded-md flex flex-row "><FaUserCircle size={15} className='mr-1 mt-[2px]'/>Enter Customber Info</button>
